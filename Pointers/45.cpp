...
//
//	45: the same including a remove function: void remove(studentCollection &head, int studentNumber)
//	Done.. I think...
//
//	Ale Zuvic, 02.02.2016
...
void removeRecord(studentCollection &head, int studentNumber);
...
void removeRecord(studentCollection &head, int studentNumber) {
	...
	while (tracker -> nextPtr != NULL) {
		if (studentNumber == tracker -> studentNumber) {
			followerTracker -> nextPtr = tracker -> nextPtr;
		}
		followerTracker = tracker;
		tracker = tracker -> nextPtr;
	}
